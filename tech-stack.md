...
| AI | OpenAI API — GPT-5 for reply drafting, GPT-5 mini for classification/routing | Full-size model quality for customer-facing drafts; the mini model is cheap and fast for high-volume classification. Structured outputs (JSON schema) for category/tags/urgency |
...
- **AI calls stay server-side.** All OpenAI API calls go through our own API endpoints — never from the browser.

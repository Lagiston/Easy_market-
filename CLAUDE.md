...
- **Role enum:** never compare or assign the role strings `"ADMIN"` / `"AGENT"` as raw literals. Server-side Prisma code (routes, `requireRole`, seeds) uses the Prisma-generated `Role` enum (`../generated/prisma/client`), which is already the source of truth for the DB column. Everywhere else — client code, shared types, test fixtures — import `Role` from `@es-market/core` (`core/src/schemas/user.ts`, a plain `{ ADMIN, AGENT }` const object mirroring the Prisma enum, since the client has no access to the Prisma-generated one) and reference `Role.ADMIN` / `Role.AGENT` instead of the string literals.

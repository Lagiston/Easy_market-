...
describe("ThemeToggle", () => {
  ...
  it("cycles light → dark → system, persisting the setting and applying the .dark class", async () => {
    ...
    await user.click(button);
    expect(localStorage.getItem("es-market-theme")).toBe("system");
    // system resolves to light under the test matchMedia stub
    expect(document.documentElement).not.toHaveClass("dark");
  });
  ...
  it("keeps the theme-color meta in sync with the resolved theme", async () => {
    ...
    await user.click(button); // system → resolves light
    await waitFor(() =>
